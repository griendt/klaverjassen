...
from models import Player, Trick, Card, Rank, Suit, Deal, RuleSet
...
class TrickTestCase(unittest.TestCase):
    ...
    def test_a_player_must_follow_suit_if_possible(self) -> None:
        ...
        self.assertEqual(True, Card(suit=Suit.HEARTS, rank=Rank.KING) in deal.players[trick.player_index_to_play].hand)
    ...
    def test_cards_can_be_compared_if_non_trump_suit_is_led(self) -> None:
        ...
        for first_rank, second_rank, second_suit in product(Rank, Rank, Suit):
            ...
            self.assertEqual(
                -1,
                trick.compare_cards(
                    Card(suit=deal.trump_suit, rank=first_rank), Card(suit=second_suit, rank=second_rank),
                ),
            )
        ...
        for first_rank, first_suit, second_rank, second_suit in product(Rank, Suit, Rank, Suit):
            if first_suit in [deal.trump_suit, trick.led_suit] or second_suit in [deal.trump_suit, trick.led_suit]:
                continue
    ...
    def test_overtrumping_your_teammate_is_not_mandatory_in_amsterdam_deals_if_teammate_is_winning(self) -> None:
        players = [Player(), Player(), Player(), Player()]
        players[0].hand = {Card(suit=Suit.HEARTS, rank=Rank.KING)}
        players[1].hand = {Card(suit=Suit.SPADES, rank=Rank.TEN)}
        players[2].hand = {Card(suit=Suit.HEARTS, rank=Rank.NINE)}
    ...
    def test_overtrumping_your_teammate_is_still_mandatory_in_amsterdam_deals_if_it_is_the_led_suit(self) -> None:
        ...
        deal = Deal(players=players, bidder_index=0, trump_suit=Suit.HEARTS, rules=RuleSet.ROTTERDAM)
        trick = Trick(deal=deal, leading_player_index=0)
